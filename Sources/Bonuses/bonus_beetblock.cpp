...
Bonus_BeetBlock::Bonus_BeetBlock(Texture* blockTexture) : Bonus_Block(blockTexture)
{
    m_output.resize(4);
...
void Bonus_BeetBlock::update()
{
    Bonus_Block::update();
}

void Bonus_BeetBlock::secureUpdate()
{
...
void Bonus_BeetBlock::draw(RenderTarget& target, RenderStates) const
{
    if (m_hit > 0 && m_hit < 3)
    {
        if (m_mushroom)
