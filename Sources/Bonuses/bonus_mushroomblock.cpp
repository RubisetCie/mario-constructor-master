...
Bonus_MushroomBlock::Bonus_MushroomBlock(Texture* blockTexture) : Bonus_Block(blockTexture)
{
    m_output.resize(4);
    m_output.setPrimitiveType(Quads);
}

void Bonus_MushroomBlock::setPosition(const Vector2f& pos)
{
    Bonus_Block::setPosition(pos);
}

void Bonus_MushroomBlock::update()
{
    Bonus_Block::update();
}

void Bonus_MushroomBlock::secureUpdate()
{
...
void Bonus_MushroomBlock::draw(RenderTarget& target, RenderStates) const
{
    if (m_hit > 0 && m_hit < 3)
