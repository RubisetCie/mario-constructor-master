...
Bonus_MushroomBlockInv::Bonus_MushroomBlockInv(Texture* blockTexture, Collider* collision, char once) : Bonus_BlockInv(blockTexture, collision, once)
{
    m_output.resize(4);
...
void Bonus_MushroomBlockInv::update()
{
    Bonus_BlockInv::update();
}

void Bonus_MushroomBlockInv::secureUpdate()
{
    ...
    switch (m_outputdir)
    {
        case 0 :
            ...
            {
                Sprite_Mushroom* item = new Sprite_Mushroom(*itemsTxt[3]);
...
void Bonus_MushroomBlockInv::hit()
{
    Vector2f pos(m_sprite.getPosition());
...
void Bonus_MushroomBlockInv::draw(RenderTarget& target, RenderStates) const
{
    if (m_hit > 0)
    {
        if (m_hit < 3)
