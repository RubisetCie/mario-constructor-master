...
Hazard_PodobooB::Hazard_PodobooB(const Texture& texture, float maxheight) : Hazard_Podoboo(texture, maxheight)
{
...
void Hazard_PodobooB::secureUpdate()
{
    if (m_timer == 70)
    {
    ...
    else if (m_timer == 69)
    {
        if (m_aabb.top < m_startheight)
        {
            ...
            listEffect.emplace_back(new Effect_LavaSplash(effectTxt[11], Vector2f(m_aabb.left - 23, m_startheight + 28), Vector2f(1, -1), 0));
        }
    }
...
Vector2f Hazard_PodobooB::getSize() const
{
    return Vector2f(27, 32);
}

void Hazard_PodobooB::draw(RenderTarget& target, RenderStates) const
{
