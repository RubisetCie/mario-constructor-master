...
Hazard_PodobooR::Hazard_PodobooR(const Texture& texture, float maxheight) : Hazard_Podoboo(texture, maxheight)
{
...
void Hazard_PodobooR::setPosition(const Vector2f& pos)
{
    ...
    float const delta(m_maxheight - m_startheight);
    if (delta < 208)
    {
...
void Hazard_PodobooR::secureUpdate()
{
    if (m_timer == 70)
    {
    ...
    else if (m_timer == 69)
    {
        if (m_aabb.left < m_startheight)
        {
            ...
            listEffect.emplace_back(new Effect_LavaSplash(effectTxt[11], Vector2f(m_startheight + 28, m_aabb.top - 23), Vector2f(1, 1), 90));
        }
    }
...
Vector2f Hazard_PodobooR::getSize() const
{
    return Vector2f(32, 27);
}

void Hazard_PodobooR::draw(RenderTarget& target, RenderStates) const
{
