...
Sprite_ParatroopaBlueH::Sprite_ParatroopaBlueH(const Texture& texture, Texture* const turtle_texture, Texture* const shell_texture, Collider* collision, float target) : Sprite_Paratroopa(texture, turtle_texture, shell_texture, collision)
{
    m_target = target + 16;
...
void Sprite_ParatroopaBlueH::flyUpdate()
{
...
void Sprite_ParatroopaBlueH::toTurtle()
{
...
ID Sprite_ParatroopaBlueH::getID() const
{
    if (m_state == 0)
        return ID_USELESS;
    else if (m_state >= 2)
...
void Sprite_ParatroopaBlueH::draw(RenderTarget& target, RenderStates) const
{
