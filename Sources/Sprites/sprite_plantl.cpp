...
#include "../../Headers/Sprites/sprite_plantl.hpp"
...
Sprite_PlantL::Sprite_PlantL(Texture* texture, Collider* collision) : Placeable()
{
...
void Sprite_PlantL::setPosition(const Vector2f& pos)
{
    m_array.append(Vertex(Vector2f(pos.x + 47, pos.y), Color::White, Vector2f(31, 0)));
    m_array.append(Vertex(Vector2f(pos.x + 47, pos.y), Color::White, Vector2f(31, 0)));
    m_array.append(Vertex(Vector2f(pos.x + 47, pos.y + 31), Color::White, Vector2f(0, 0)));
    m_array.append(Vertex(Vector2f(pos.x + 47, pos.y + 31), Color::White, Vector2f(0, 0)));

    m_startwidth = pos.x;

    m_aabb.left = pos.x+47;
    m_aabb.top = pos.y;
}

void Sprite_PlantL::update()
{
    if (frame_Flower % 8 == 0)
    {
        m_array[0].texCoords.x = (frame_Flower/8) * 31 + 31;
        m_array[1].texCoords.x = (frame_Flower/8) * 31 + 31;
        m_array[2].texCoords.x = (frame_Flower/8) * 31;
...
Vector2f Sprite_PlantL::getPosition() const
{
    return Vector2f(m_startwidth, m_aabb.top);
}

Vector2f Sprite_PlantL::getSize() const
{
    return Vector2f(47, 31);
}

ID Sprite_PlantL::getID() const
{
    return ID_USELESS;
}
