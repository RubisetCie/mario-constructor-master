...
Sprite_ParatroopaRedJ::Sprite_ParatroopaRedJ(const Texture& texture, Texture* const turtle_texture, Texture* const shell_texture, Collider* collision) : Sprite_Paratroopa(texture, turtle_texture, shell_texture, collision)
{
}

void Sprite_ParatroopaRedJ::flyUpdate()
{
    if (!m_active)
        return;
...
void Sprite_ParatroopaRedJ::toTurtle()
{
}

ID Sprite_ParatroopaRedJ::getID() const
{
    if (m_state == 0)
        return ID_STOMPABLE;
    else if (m_state >= 2)
...
void Sprite_ParatroopaRedJ::draw(RenderTarget& target, RenderStates) const
{
