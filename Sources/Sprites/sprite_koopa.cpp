...
#include "../../Headers/Sprites/sprite_koopa.hpp"
...
Sprite_Koopa::Sprite_Koopa(const Texture& sprite_texture, Texture* const shell_texture, Collider* collision, bool shell) : Walkable()
{
    if (shell)
    {
...
void Sprite_Koopa::update()
{
    ...
    if (blockkiller.intersects(m_aabb))
    {
        ...
        addPoints(100);
        listEffect.emplace_back(new Effect_Score(effectTxt[13], killcount, Vector2f(m_aabb.left-5, m_aabb.top+16)));
...
void Sprite_Koopa::secureUpdate()
{
    if (m_state != 1)
    {
        if (m_state == 0)
...
void Sprite_Koopa::afterUpdate()
{
}

Vector2f Sprite_Koopa::getPosition() const
{
    return Vector2f(m_aabb.left, m_aabb.top-15);
}

Vector2f Sprite_Koopa::getSize() const
{
    return Vector2f(32, 47);
}

ID Sprite_Koopa::getID() const
{
    if (m_state < 2)
    {
        if (m_collider->type == C_STOMPABLE)
