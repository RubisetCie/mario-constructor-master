...
#define PARATROOPA_BOUNCE 8
#define PARATROOPA_MAXVSPEED 10

using namespace sf;
using namespace std;

Sprite_ParatroopaGreenJ::Sprite_ParatroopaGreenJ(const Texture& texture, Texture* const turtle_texture, Texture* const shell_texture, Collider* collision) : Sprite_Paratroopa(texture, turtle_texture, shell_texture, collision)
{
}

void Sprite_ParatroopaGreenJ::flyUpdate()
{
    if (!m_active)
        return;
...
void Sprite_ParatroopaGreenJ::toTurtle()
{
}

ID Sprite_ParatroopaGreenJ::getID() const
{
    if (m_state == 0)
        return ID_STOMPABLE;
    else if (m_state >= 2)
...
void Sprite_ParatroopaGreenJ::draw(RenderTarget& target, RenderStates) const
{
