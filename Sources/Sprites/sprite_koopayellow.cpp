...
#define KOOPAYELLOW_MAXHSPEED 4.1
#define KOOPAYELLOW_ACCELERATION 0.11

using namespace std;
using namespace sf;

Sprite_KoopaYellow::Sprite_KoopaYellow(const Texture& sprite_texture, Texture* const shell_texture, Collider* collision, bool shell) : Walkable()
{
    if (shell)
    {
        m_sprite.setTexture(*shell_texture);
...
void Sprite_KoopaYellow::update()
{
    m_active = true;
    ...
    {
        if (frame_Flower % 8 == 0)
            m_sprite.setTextureRect(IntRect((frame_Flower/8) * 32, 0, 32, 47));
        ...
        if (player != NULL)
        {
            ...
            if (pos>=playerpos+14)
            {
                if (m_movedistance.x > -KOOPAYELLOW_MAXHSPEED)
                    m_movedistance.x -= KOOPAYELLOW_ACCELERATION;
            }
            else
            {
                if (m_movedistance.x < KOOPAYELLOW_MAXHSPEED)
...
void Sprite_KoopaYellow::secureUpdate()
{
    if (!m_active)
        return;
...
void Sprite_KoopaYellow::afterUpdate()
{
}

Vector2f Sprite_KoopaYellow::getPosition() const
{
    return Vector2f(m_aabb.left, m_aabb.top-15);
}

Vector2f Sprite_KoopaYellow::getSize() const
{
    return Vector2f(32, 47);
}

ID Sprite_KoopaYellow::getID() const
{
    if (m_state < 2)
    {
        if (m_collider->type == C_STOMPABLE)
...
void Sprite_KoopaYellow::draw(RenderTarget& target, RenderStates) const
{
