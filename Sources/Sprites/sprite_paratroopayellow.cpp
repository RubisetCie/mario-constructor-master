...
#define PARATROOPA_MAXSPEED 3.5
#define PARATROOPA_MAXHSPEED 4.1
#define PARATROOPA_MAXVSPEED 12
#define PARATROOPA_ACCELERATION 0.11

using namespace sf;
using namespace std;

Sprite_ParatroopaYellow::Sprite_ParatroopaYellow(const Texture& texture, Texture* const turtle_texture, Texture* const shell_texture, Collider* collision) : Pawn()
{
...
void Sprite_ParatroopaYellow::update()
{
    m_active = true;
    ...
    if (m_state < 3)
    {
        ...
        if (player != NULL)
        {
            if (m_state == 0)
            {
                float angle = atan2(m_aabb.top - (player->m_aabb.top + 6), m_aabb.left - (player->m_aabb.left + 2));
                Vector2f target(-PARATROOPA_MAXSPEED * cos(angle), -PARATROOPA_MAXSPEED * sin(angle));

                if (target.x > 0)
                {
                    if (m_movedistance.x < target.x)
                        m_movedistance.x += PARATROOPA_ACCELERATION;
                }
                else
                {
                    if (m_movedistance.x > target.x)
                ...
                if (target.y > 0)
                {
                    if (m_movedistance.y < target.y)
                        m_movedistance.y += PARATROOPA_ACCELERATION;
                }
                else
                {
                    if (m_movedistance.y > target.y)
            ...
            else if (m_state == 1)
            {
                ...
                if (pos>=playerpos+14)
                {
                    if (m_movedistance.x > -PARATROOPA_MAXHSPEED)
                        m_movedistance.x -= PARATROOPA_ACCELERATION;
                }
                else
                {
...
void Sprite_ParatroopaYellow::secureUpdate()
{
    if (m_state != 2)
    {
...
void Sprite_ParatroopaYellow::walkUpdate()
{
    if (!m_active)
        return;
    ...
    if (m_state == 0)
        m_sprite.setPosition(m_aabb.left+16, m_aabb.top-16);
    else if (m_state == 1)
        m_sprite.setPosition(m_aabb.left+16, m_aabb.top-15);
    else
...
void Sprite_ParatroopaYellow::flyUpdate()
{
    if (!m_active)
        return;
...
void Sprite_ParatroopaYellow::afterUpdate()
{
}

Vector2f Sprite_ParatroopaYellow::getPosition() const
{
    return Vector2f(m_aabb.left, m_aabb.top-16);
}

Vector2f Sprite_ParatroopaYellow::getSize() const
{
    return Vector2f(32, 48);
}

ID Sprite_ParatroopaYellow::getID() const
{
    if (m_state == 0)
        return ID_STOMPABLE;
    else if (m_state < 2)
...
void Sprite_ParatroopaYellow::draw(RenderTarget& target, RenderStates) const
{
