...
Sprite_Goomba::Sprite_Goomba(const Texture& texture, Collider* collision) : Walkable()
{
...
void Sprite_Goomba::secureUpdate()
{
    if (!m_active)
        return;
...
void Sprite_Goomba::afterUpdate()
{
}

Vector2f Sprite_Goomba::getPosition() const
{
    return Vector2f(m_aabb.left, m_aabb.top);
}

Vector2f Sprite_Goomba::getSize() const
{
    return Vector2f(31, 32);
}

ID Sprite_Goomba::getID() const
{
    if (m_state == 0)
        return ID_ENEMY;
    else
...
void Sprite_Goomba::draw(RenderTarget& target, RenderStates) const
{
