...
Sprite_SpinyGrey::Sprite_SpinyGrey(const Texture& texture, Collider* collision) : Walkable()
{
...
void Sprite_SpinyGrey::secureUpdate()
{
    if (!m_active)
        return;
...
void Sprite_SpinyGrey::afterUpdate()
{
}

Vector2f Sprite_SpinyGrey::getPosition() const
{
    return Vector2f(m_aabb.left, m_aabb.top);
}

Vector2f Sprite_SpinyGrey::getSize() const
{
    return Vector2f(33, 32);
}

ID Sprite_SpinyGrey::getID() const
{
    return ID_ENEMY;
}

void Sprite_SpinyGrey::draw(RenderTarget& target, RenderStates) const
{
