...
Sprite_ParatroopaBlueV::Sprite_ParatroopaBlueV(const Texture& texture, Texture* const turtle_texture, Texture* const shell_texture, Collider* collision, float target) : Sprite_Paratroopa(texture, turtle_texture, shell_texture, collision)
{
    m_target = target;
...
void Sprite_ParatroopaBlueV::setPosition(const Vector2f& pos)
{
    m_startpos = pos.y;
...
void Sprite_ParatroopaBlueV::flyUpdate()
{
    if (player != NULL)
    {
...
void Sprite_ParatroopaBlueV::toTurtle()
{
...
ID Sprite_ParatroopaBlueV::getID() const
{
    if (m_state == 0)
        return ID_STOMPABLE;
    else if (m_state >= 2)
...
void Sprite_ParatroopaBlueV::draw(RenderTarget& target, RenderStates) const
{
