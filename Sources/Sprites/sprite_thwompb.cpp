...
Sprite_ThwompB::Sprite_ThwompB(const Texture& texture) : Pawn()
{
...
void Sprite_ThwompB::setPosition(const Vector2f& pos)
{
    m_sprite.setPosition(pos);

    m_startheight = pos.y + 4;

    m_aabb.left = pos.x + 2;
    m_aabb.top = m_startheight;
}

void Sprite_ThwompB::update()
{
    if (m_laugh > 0)
    {
        if (m_laugh % 4 == 0)
...
void Sprite_ThwompB::secureUpdate()
{
    if (m_state == 1)
    {
        ...
        if (zoneb)
        {
            ...
            if (makeMoveVerticalHigh(m_movedistance, *currentScale, collisionMatrixb, &listColliderb))
            {
                ...
                spritehitter.top = m_aabb.top+60;
                spritehitter.width = 50;
                spritehitter.height = 8;
                spritehitter_create = false;

                listEffect.emplace_back(new Effect_Fire(effectTxt[9], Vector2f(m_aabb.left - 7, m_aabb.top + 49)));
                listEffect.emplace_back(new Effect_Fire(effectTxt[9], Vector2f(m_aabb.left + 26, m_aabb.top + 49)));
    ...
    m_sprite.setPosition(m_aabb.left - 2, roundf(m_aabb.top - 4));
    ...
    if (m_state == 0)
    {
        ...
        if (player->m_aabb.top > m_aabb.top && player->m_aabb.left > m_aabb.left - 70 && player->m_aabb.left < m_aabb.left + 92)
            m_state = 1;
    }
...
void Sprite_ThwompB::afterUpdate()
{
}

Vector2f Sprite_ThwompB::getPosition() const
{
    return Vector2f(m_aabb.left - 2, m_aabb.top - 4);
}

Vector2f Sprite_ThwompB::getSize() const
{
    return Vector2f(58, 68);
}

ID Sprite_ThwompB::getID() const
{
    return ID_USELESS;
}

void Sprite_ThwompB::draw(RenderTarget& target, RenderStates) const
{
