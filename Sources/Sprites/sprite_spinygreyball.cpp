...
Sprite_SpinyGreyBall::Sprite_SpinyGreyBall(const Texture& texture, const Texture& walktexture, Collider* collision, Sprite_Lakitu* parent) : Pawn()
{
...
void Sprite_SpinyGreyBall::update()
{
    if (m_state == 1)
    {
        if (frame_Flower % 8 == 0)
...
void Sprite_SpinyGreyBall::secureUpdate()
{
    if (m_state == 1)
    {
...
void Sprite_SpinyGreyBall::afterUpdate()
{
}

Vector2f Sprite_SpinyGreyBall::getPosition() const
{
    return Vector2f(m_aabb.left, m_aabb.top);
}

Vector2f Sprite_SpinyGreyBall::getSize() const
{
    return Vector2f(33, 32);
}

ID Sprite_SpinyGreyBall::getID() const
{
    if (m_state == 0)
        return ID_USELESS;
    else
...
void Sprite_SpinyGreyBall::draw(RenderTarget& target, RenderStates) const
{
